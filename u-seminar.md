...
## 沿革
政治学演習（栗崎周平）は2013年から始まり、2021-2023年はサバティカルのため休止し、2026年度は11期生、2027年度は12期生を迎えます。このゼミのテーマは2軸あります：モデル×実証的（国際）政治。広く「実証的あるいは分析的な（国際）政治学研究」を行うと同時にゼミ生は各自のモデルを制作します。

## ゼミのポリシー
教員によるテーマは設定せず学生プロジェクトを推進するワークショップ型のセミナーとして運営してきました。ゼミのポリシーは以下です
- 5年後、10年後を見据えて戦略的に毎学期の活動目的と内容を（再）定義をする（Write Your Storyという課題がプレゼミ開始時と4年生卒業時にあります）
- 次の３つのモデルから一つ（一つ以上でもOK）を選択し作品を作成する
  - 数理モデル：ゲーム理論を使った理論分析を行い、論文ないしポスターを作品とする
  - データモデリング：データ科学ないし統計分析を使った経験モデルのことで、論文ないしポスターをその作品とする（非構造化データの収集・可視化などデータエンジニアリング的な作品でもOK）
  - ビジネスモデル：ビジネスモデルを策定し、卒業までに実際に起業した事業が提供するプロダクトそのものを作品とするか、VCからのファンド出資を得るためのピッチデックを作品とする
- ゼミ合宿などのエクストラカリキュラーアクティビティは教員側は特に用意しませんが、ゼミ生の自主的な企画にお任せします（ただし3月の海外学会出張は実質的なゼミ旅行：この場合、渡航費＋宿泊費は教員負担）。
- 自分の活動にOwnershipを持って自分の人生をコントロールする

## ゼミの傾向
- 栗崎ゼミは多い時でも一学年7名と応募が少ないですが、主体性を持った卒業生が多いことが特徴です。[卒業生リスト](./teaching-j.html#123)に示しましたが、進路については第一志望に進むことと、目的意識を持ってキャリアのフェーズごとに移籍する人が多いことも特徴です。その結果、起業したCEO、研究者（企業・大学教員）、海外駐在が多いという特徴もあります。
- 栗崎が創業した事業に参画ないしコラボしたり、栗崎研究室と共同研究を行うなど、研究・事業・ビジネスのコラボが多く発生していることも特徴です。
- ロンドンをベースとするコンサルファームがインターン（長期有償）を栗崎ゼミから継続的にリクルートしています（2023年以降）。

## ゼミ在籍生、卒業生、修了生
OBOGリストと彼・彼女たちの進路や現在のポシジョンなどのリストは[教育・授業ページ](./teaching-j.html) の下段をご覧ください。

# 研究テーマ
ゼミテーマとして起業に取り組んでいるのは現在は2名（4年生、3年生、各１名）。その他は、数理モデルないしデータ分析に取り組んでいます。そこでは広く「実証的・分析的な政治学研究」を行う限りにおいて、研究テーマは参加学生が自由に設定しています。担当教員は国際政治学における理論研究（ゲーム理論分析）ないし実証研究（データ科学）を行うことを得意としていますが、広義の政治学の範疇で、政治問題、社会問題、経済問題、秩序問題、不正義問題、政策問題などから課題や関心を説き起こし、モデル分析やデータ科学の手法を駆使する研究に関心がある方を歓迎します。

これまでのゼミ生による研究が採用した分析手法の割合は、従来型の計量分析が5割、データ科学や因果推論を意欲的に導入するものが2割、ゲーム理論を用いた数理的な理論研究がおよそ2割です。分析対象の内訳は安全保障が約4割、国際政治のその他に関する現象の分析が3割、国際政治経済に係るものが2割、比較政治に部位類されるものが1割です。これまでのゼミ生による研究の例を以下に列挙します。括弧内は卒業後の進路です。
- 民主主義の後退democratic backslidingのゲーム理論分析（東大院経済→ノースウェスタン経済PhD）
...
- Okinawa’s Economic Growth and U.S. Military Deployment: Panel Analysis at the Municipal Level (財務省総合職→LSE修士)<br>

これまでのゼミ生が起こした事業
- 全国の事業所の従業員シフト作成・運用の課題を解決する事業（クロスビットXbit, 創業10年で成功裡にM&A（会社売却））
- 生産者と消費者を直接繋ぐECプラットフォームにおける情報の非対称を克服する事業（現役ゼミ生）
- メンタルヘルスサービスのロバストな提供を通したメンタルヘルスの民主化を求める事業（現役ゼミ生）
- 経済安全保障と開放政治経済を両立させ保護主義への後退を回避するためのAI駆動型のインテリジェンス・データを提供する事業（教員）
...
- 演習I/Advanced Seminar I（3年生春学期）
  - 研究課題設定のブレスト
  - 研究関心分野の既存研究サーベイと自分の研究課題の距離の描写（ビジネスモデル策定の場合は、解決すべき社会課題、助ける顧客、既存の解決策、自社プロダクトのシードに関するのサーベイ）

- 演習II/Advanced Seminar II（3年生秋学期）
  - 先行研究の再現分析とリサーチデザイン策定（トーイモデル）（先行ビジネスの再現と改善策）
  - 必要な場合はデータ収集とデータ整理（チーム組成、プロダクト作り、顧客ヒヤリング、POC、PMF）

- 演習III/Advanced Seminar III（4年生春学期）
  - 分析継続、分析結果の解釈、頑強性チェック、議論・命題の確定（出資者プレゼン、MVP minimum viable product 作成）
  - 就活生は就活に全力
...
- 海外出張
  - 2026-27年年度生（2028年3月卒業：現在3年生）は以下２つの選択肢
    - ISA 2028, New Orleans, Louisiana, March 1-4
    - WPSA 2028, San Diego (Manchester Grand Hyatt), April 13-15 (就職組は日程的に困難)
  - 2027-28年年度生（2029年3月卒業：現在2年生）は以下２つの選択肢
    - WPSA 2029, Austin (Hilton Austin), March 29-31
    - ISA 2029, Las Vegas, March 26-29　 (採択競争率高めだが卒業式翌日からラスベガスに公費で出張)
  - 2028-29年年度生（2030年3月卒業：現在1年生）は以下２つの選択肢
    - WPSA 2029, 未定, 未定
    - ISA 2030, Baltimore, February 17-20
...
- <b>選考方法</b>: 提出は二箇所に必要です<br>
  ...
  - 栗崎への個別課題提出 at kurizaki@waseda.jp (Zoom面接は必要に応じて行います)<br>

- <b>申請期間・提出期限:</b> 応募申請、個別課題ともに、政経学部事務所が設定する提出期限を踏襲します＜2026年度募集の場合は以下＞
  - 第1回選考
